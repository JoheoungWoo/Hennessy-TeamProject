...
String userID = null; 
String selInf = "SELECT board_user FROM board_table WHERE board_id = ?";
String isAdmin = null;
...
if (session.getAttribute("userID") != null) {
	...
	try {
		String dbURL = "jdbc:mysql://localhost:3306/game_schema";
		String dbID = "root";
...
} // 세션 영역에 userID가 존재한다면 값을 불러와 저장함 + DB 호출
%>
<%
String boardUser = null;
int boardID = 0;
if (request.getParameter("boardID") != null) {
	boardID = Integer.parseInt(request.getParameter("boardID"));
	try {
		pstmt = conn.prepareStatement(selInf);
		pstmt.setInt(1, boardID);
		rs = pstmt.executeQuery();
		while(rs.next()){  				
			boardUser = (String) rs.getString(1);
		}
	}catch(Exception e) {
		e.printStackTrace();
	}
...
if ((!boardUser.equals(userID)) && (!isAdmin.equals("YES"))) {
	PrintWriter script = response.getWriter();
	script.println("<script>");
    ...
    <div class="main_split">
      ...
      <div class="main_area">
		<div id="board_area" class="jsx-2303464893 editor">
			<form action="updateAction.jsp?boardID=<%= boardID %>" method="post"  enctype="multipart/form-data">
				<div id="board_tag">
					<select name="b_tag" id="select">
						<option value="">-----</option>
				...
				<div id="board_title">
					<input type="text" name="b_title" id="title" placeholder="제목을 입력해 주세요." value="<%= board.getB_title() %>">
				</div>
				<div class="fr-box fr-basic fr-top" role="application" style="background-color:white;">
					<div class="fr-wrapper show-placeholder" dir="auto" style="overflow: scroll;">
						<textarea name="b_content" id="smartEditor"
							style="width:100%; height: 412px; "><%= board.getB_content() %></textarea>
					</div>
				</div>
